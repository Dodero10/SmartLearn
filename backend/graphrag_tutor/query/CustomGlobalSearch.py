...
from .CustomSystemPrompt import (
...
class CustomGlobalSearch(GlobalSearch):
    def __init__(
        self,
        llm: BaseLLM,
        ...
        **llm_kwargs,
        ...
        try:
            ...
            if len(filtered_key_points) == 0 and not self.allow_general_knowledge:
                ...
                detection_messages = [
                    {"role": "system", "content": GREETING_DETECTION_PROMPT},
                    {"role": "user", "content": query},
                ]
                detection_response = await self.llm.agenerate(
                    detection_messages,
                    streaming=True,
                    callbacks=self.callbacks,  # type: ignore
                    **llm_kwargs,  # type: ignore
                )
                if detection_response == "0":
                    log.warning(
                        "Warning: Greeting"
                    )
                    greeting_messages = [
                    ...
                    greeting_response = await self.llm.agenerate(
                        greeting_messages,
                        streaming=True,
                        callbacks=self.callbacks,  # type: ignore
                        **llm_kwargs,  # type: ignore
                    ...
                    final_response = "Xin lỗi tôi không có để thông tin để trả lời câu hỏi trên."
                    local_query = LocalQuery()
                    _, question_generator = local_query.get_search_engine()
    ...
    async def asearch(
        ...
        **kwargs: Any,
        ...
        ), num_tokens(reduce_response.response, self.token_encoder)
